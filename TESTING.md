...
| **Protocol self-test** | The real `mojoudfclient` binary speaks the full ZMQ + protobuf `MT_*` exchange correctly for every UDF/wire combination, including multi-batch group accumulation. Covers all three UDF shapes: **SCALAR** (`DOUBLE_MOJO`, `PY_SCALE`), **SET** (`SUM_POSITIVE`), and **EMITS** — one-to-many (`MIRROR_MOJO`, 2 output rows per input). | [`test/fake_exasol.py`](test/fake_exasol.py) driven from the Dockerfile `selftest` stage | `docker build --target selftest` |
