...
# test/language_definitions_test.sh against one fixture per defect class, under
# test/fixtures/language_definitions/. Each negative fixture is an
# otherwise-conforming document varying exactly one fact, so a non-zero exit on
# it is evidence that the corresponding assertion in
# language_definitions_test.sh actually discriminates, not just that it exists.
# conforming.json proves the same script accepts a fully correct document.
#
# The driver owns these pass/fail assertions; language_definitions_test.sh keeps
# exactly one subject (the document passed as its argument) and is never edited
# to know about fixtures.
#
# Ported from exasol-labs/language-container-rs
# (dist/tests/language_definitions_fixtures_test.sh).
#
# Run: bash test/language_definitions_fixtures_test.sh
set -uo pipefail
...
# One case per line, in the order the contract script asserts them, so a missing
# branch is visible as a gap in the sequence.
