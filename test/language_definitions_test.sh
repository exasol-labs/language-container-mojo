...
# build_info/language_definitions.json document against the Exasol database's
# language_definitions v2 metadata schema, for the native Mojo SLC.
#
# Path-addressed jq assertions, not substring matching, so a renamed root key
# or a re-typed field is caught instead of silently passing.
#
# Sole owner of the DB SLC v2 contract for this repo: both the pre-build source
# check (run directly on build_info/language_definitions.json) and the
# post-build artifact check (test/slc_tarball_test.sh, on the extracted tarball
# copy) delegate to this script, so the source and the shipped copy can never
# state different schemas.
#
# Ported from exasol-labs/language-container-rs
# (dist/tests/language_definitions_test.sh); adapted to the Mojo definition
# (aliases ["MOJO"], lang=mojo, executable /exaudf/mojoudfclient).
#
# Run: bash test/language_definitions_test.sh <document.json>
set -uo pipefail
...
language_definitions_holds_one_conforming_definition() {
    ...
    if ! jq -e '.language_definitions[0].aliases == ["MOJO"]' "$DOC" >/dev/null 2>&1; then
        fail "language_definitions_holds_one_conforming_definition: .language_definitions[0].aliases is not [\"MOJO\"]"
        return
    fi
    ...
    if ! jq -e '.language_definitions[0].parameters == [{"key": "lang", "value": "mojo"}]' "$DOC" >/dev/null 2>&1; then
        fail "language_definitions_holds_one_conforming_definition: .language_definitions[0].parameters is not [{\"key\": \"lang\", \"value\": \"mojo\"}]"
        return
    fi
    if ! jq -e '.language_definitions[0].udf_client_path.executable == "/exaudf/mojoudfclient"' "$DOC" >/dev/null 2>&1; then
        fail "language_definitions_holds_one_conforming_definition: .language_definitions[0].udf_client_path.executable is not \"/exaudf/mojoudfclient\""
        return
    fi
    # jq renders an absent key and a null value identically, so presence and
    # value must be asserted as two separate checks — has("deprecation") alone
    # would still pass a document that never mentions the key.
...
# Every assertion below addresses the document by jq key path, and jq reports an
# unparseable document the same way it reports an absent key. Running them on a
# document that does not parse would turn one syntax error into several messages
# naming fields the document may well declare correctly, so they run only once
# the document is known to be valid JSON.
language_definitions_document_is_valid_json
